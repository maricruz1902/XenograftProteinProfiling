...
PlotTargets(cellLine = c(6), 
            treatment = c("(+)-Serum", "Dacomitinib", "Dactolisib", 
                          "Buparlisib", "DMSO"), 
            timePoint = c(1, 12, 24, 48))

# Format edges
CustomGraph <- function(graph) {
        E(graph)$cor <- E(graph)$weight
        E(graph)$weight <- abs(E(graph)$cor)
        E(graph)$color <- ifelse(E(graph)$cor < 0, "black", "red")
...
colors <- rep(colorList, each = 4)

V(graph)$color <- colors

graph <- delete.edges(graph, E(graph)[ abs(weight) < 0.5 ])
...
corrplot(holder.r$r, method = "color", mar= c(1,1,1,1), tl.col = "black", tl.cex = 1)

hmcol <- colorRampPalette(c("darkblue", "white", "darkred"))(256)

heatmap(holder.r$r, col = hmcol, mar = c(10, 10), scale = "none")

PlotTreatments(cellLine = c(6, 10), 
               treatment = c("Dacomitinib", "Dactolisib", 
                             "Buparlisib", "DMSO"),
               timePoint = c(1, 24))
CustomGraph(graph = g)
...
V(g)$color <- ifelse(grepl("6", V(g)$name), "#ffffbf", "#91bfdb")
...
heatmap(holder.r$r, col = hmcol, scale = "none")
