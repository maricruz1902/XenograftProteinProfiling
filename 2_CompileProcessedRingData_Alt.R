...
CompileData <- function(){
        ...
        for (i in 1:length(listData)){
                ...
                dat <- read_csv(paste("plots/", names, 
                        "_netShifts.csv", sep=""), col_types = cols())
...
CombinedAveraged <- function(filename = "compiledLabeled.csv"){
        ...
        dat$groupName <- gsub('_ch1', '', dat$groupName)
        dat$groupName <- gsub('_ch2', '', dat$groupName)
        ...
        # divide to account for duplication below
        len.cellLine <- len.tot / length(cellLine) / 2
        len.treatment <- len.tot / length(treatment) / 4
        len.timePoint <- len.tot / length(timePoint)
        
        # create vectors for matrix for subsequent data averaging
        targets <- rep(targets, each = len.targets)
        timePoint <- rep(timePoint, len.timePoint)
        treatment <- rep(treatment, each = 4)
        treatment <- rep(treatment, len.treatment)
        cellLine <- rep(unique(dat$`Cell Line`), each = 2)
        ...
        allGroups <- filter(allGroups, targets != "thermal" & 
                                    treatment != "Blank")
        ...
        for(i in 1:nrow(allGroups)){
                ...
                dat.group <- filter(dat, groupName == id.tar, `Cell Line` == 
                                            id.cell & `Time Point` == id.time & 
                                            Treatment == id.treat)
        ...
        # saves data
        write_csv(df, "combinedAveraged.csv")
}
