...
dat <- rename(dat, "TimePoint" = "Time Point", "CellLine" = "Cell Line",
              "NetShift" = "Net Shift")
...
shapiro.test(log(datScaled$NetShift))
...
cast.gbm6 <- filter(cast, CellLine == "GBM 6")
cast.gbm26 <- filter(cast, CellLine == "GBM 26")
...
cast.alt <- dcast(data = datSum, Treatment + CellLine + TimePoint ~
                          Target, mean,
                  value.var = "Net Shift")
...
man <- manova(cbind(targetList[1], targetList[2]) ~ 
                      Treatment + 
                      CellLine +
                      TimePoint + 
                      Treatment * CellLine + 
...
scaleCols <- scale(select(cast, -Treatment, -CellLine,
                           -TimePoint, -Replicate, -n), center = T)
qqnorm(scaleCols); qqline(scaleCols)
labelCols <- select(cast, Treatment, CellLine, TimePoint, Replicate, n)
